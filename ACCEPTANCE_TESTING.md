...
| `scripts/nginx_smoke.sh` | dockerized nginx 同域入口 + `/api` 代理 + 安全响应头校验 |
...
1. `scripts/validate_production_env.sh`（正反例）
...
7. `scripts/intranet_acceptance.sh`（增量，非冷启动）
